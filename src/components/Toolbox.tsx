...
const Toolbox: React.FC = () => {
  ...
  const spatialAnalysisTools = [
    { name: '叠置分析', icon: 'overlay' },
    { name: '高级叠置分析', icon: 'advancedOverlay' },
    { name: '缓冲区分析', icon: 'buffer' },
    { name: '多重缓冲区分析', icon: 'multiBuffer' },
    { name: '非对称缓冲区分析', icon: 'asymmetricBuffer' },
    { name: '字段缓冲区分析', icon: 'fieldBuffer' },
    { name: '空间关系分析', icon: 'spatialRelation' },
    { name: '数据检查', icon: 'dataCheck' },
  ];

  const geometryAnalysisTools = [
    { name: '投影系统转换', icon: 'projection' },
  ];

  const mapOutputTools = [
    { name: '截图', icon: 'screenshot' },
  ];
            ...
            {expandedSections.includes('空间分析') && (
              <div className="px-3 pb-3 grid grid-cols-2 gap-1">
                {spatialAnalysisTools.map((tool, idx) => (
                  <button 
                    key={idx} 
                    className="text-left text-xs px-2 py-2 rounded hover:bg-blue-50 hover:text-blue-700 text-gray-600 flex items-center gap-2 transition-colors"
                  >
            ...
            {expandedSections.includes('几何分析') && (
              <div className="px-3 pb-3">
                {geometryAnalysisTools.map((tool, idx) => (
                  <button 
                    key={idx} 
                    className="text-left text-xs px-2 py-2 rounded hover:bg-blue-50 hover:text-blue-700 text-gray-600 flex items-center gap-2 transition-colors w-full"
                  >
                    {renderIcon(tool.icon)}
